...
Flags in `$ARGUMENTS`:
- `-y` / `--yes` — one yes for the whole loop. Show the first question once, wait for yes, then send that question and up to two follow-ups with `-y` on each send, without asking again between them. Say that up front.
- No task (bare `/acq`, or only flags) — run `ready` and stop.

Rules:
- Nothing is sent without a yes. On a plain `/acq <task>`, that is one yes per send. On `/acq -y <task>`, that is one yes for the run.
